  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const porConsultor = Object.values(consultorMap).map(c => ({
        ...c,
        capital: c.capital,
        txConversao: c.total > 0 ? ((c.vendidos/c.total)*100).toFixed(1) : '0.0'
      ...
      const porMarca = Object.values(marcaMap).map(m => ({
        ...m,
        txConversao: m.total > 0 ? ((m.vendidos/m.total)*100).toFixed(1) : '0.0'
      ...
      const temporal = Array.from({ length: dias }, (_,i) => {
        ...
        return {
          ...
          vendidos: vendidosNoDia.length,
          capital:  soma(vendidosNoDia),
        }
      })
      ...
      return {
        ...
        motivosPerda, porRegiao, scoreDist,
        leads: rows,
      }
    }
  })
...
export function useFiltrosRelatorio(tenantId) {
  return useQuery({
    ...
    enabled: !!tenantId,
    queryFn: async () => {
      const [marcas, operadores, status] = await Promise.all([
        supabase.from('marcas').select('id,nome,emoji').eq('tenant_id', tenantId).eq('ativo', true),
        supabase.from('usuarios').select('id,nome').eq('tenant_id', tenantId).eq('active', true),
