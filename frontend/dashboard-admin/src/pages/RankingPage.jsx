...
export default function RankingPage() {
  ...
  useEffect(() => {
    async function carregar() {
      setLoading(true)
      try {
        // Busca leads do ms com operador_id
            ...
            <div className="bg-[#0B1220] border border-white/[0.06] rounded-2xl overflow-hidden">
              ...
              {consultores.length === 0 ? (
                <div className="py-16 text-center">
                  <p className="text-gray-600 text-sm">Nenhum dado para o perodo selecionado</p>
                  <p className="text-gray-700 text-xs mt-1">
                    Os leads precisam ter o campo <code className="text-[#10B981]">operador_id</code> preenchido
                  </p>
                </div>
              ) : (
                consultores.map((c, i) => (
