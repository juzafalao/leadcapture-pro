// RankingPageEnhanced.jsx — Ranking de Consultores com Gráficos e Relatórios Avançados
// Versão 2.0 com suporte a gráficos, filtros e análises detalhadas
// Paleta oficial: #0F172A fundo, #10B981 verde/ativo, cinzas
// 
// NOVAS FUNCIONALIDADES:
// 1. Gráficos de evolução (recharts)
// 2. Filtros avançados (tipo de lead, fonte, marca)
// 3. Relatórios detalhados com exportação
// 4. Análises de performance por período
// 5. Comparativo entre consultores
...
function RankRow({ consultor, pos, meta }) {
  ...
  const txConv = consultor.total_leads > 0 
    ? Math.round((consultor.convertidos / consultor.total_leads) * 100) 
    : 0
  ...
  return (
    ...
    >
      ...
      <div className="text-right shrink-0 hidden sm:block">
        <p className="text-[11px] font-bold text-[#10B981]">{consultor.leads_hot}</p>
        <p className="text-[9px] text-gray-600">hot</p>
      </div>
      <div className="text-right shrink-0 hidden sm:block">
        <p className="text-[11px] font-bold text-[#10B981]">{txConv}%</p>
...
// Página Principal
export default function RankingPageEnhanced() {
  ...
  const [filtros, setFiltros]   = useState({
    categoria: 'todos', // hot, warm, cold, todos
    minLeads: 0,
    maxLeads: 999,
  })
  ...
  useEffect(() => {
    async function carregar() {
      setLoading(true)
      try {
        // Busca leads do mês com operador_id
        ...
        let qLeads = supabase
          .from('leads')
          .select('id, score, categoria, capital_disponivel, operador_id, status, created_at')
        ...
        // Busca usuários (consultores)
        let qUsers = supabase
          .from('usuarios')
          .select('id, nome, role, role_emoji, role_color')
          .in('role', ['Consultor','Gestor','Diretor','Administrador'])
        if (tenantId) qUsers = qUsers.eq('tenant_id', tenantId)
        const { data: users } = await qUsers
        ...
        for (const lead of (leads || [])) {
          const uid = lead.operador_id
          if (!uid) continue
          if (!mapaLeads[uid]) mapaLeads[uid] = { total: 0, hot: 0, warm: 0, cold: 0, convertido: 0, capital: 0 }
          mapaLeads[uid].total++
          if (lead.categoria === 'hot')         mapaLeads[uid].hot++
          else if (lead.categoria === 'warm')   mapaLeads[uid].warm++
          else                                  mapaLeads[uid].cold++
        ...
        const ranking = users
          .map(u => ({
            ...
            leads_warm:   mapaLeads[u.id]?.warm      ?? 0,
            leads_cold:   mapaLeads[u.id]?.cold      ?? 0,
  ...
  const consultoresFiltrados = useMemo(() => {
    return consultores.filter(c => {
      if (filtros.categoria !== 'todos') {
        const leads = filtros.categoria === 'hot' ? c.leads_hot : 
                     filtros.categoria === 'warm' ? c.leads_warm : c.leads_cold
        if (leads === 0) return false
      }
      if (c.total_leads < filtros.minLeads || c.total_leads > filtros.maxLeads) return false
      return true
    })
  }, [consultores, filtros])

  const top3 = consultoresFiltrados.slice(0, 3)

  // Exportar CSV
  function exportCSV() {
    const header = 'Posicao,Nome,Leads,HOT,WARM,COLD,Convertidos,Taxa Conversao,Capital\n'
    const rows   = consultoresFiltrados.map((c, i) => {
      const txConv = c.total_leads > 0 ? Math.round((c.convertidos / c.total_leads) * 100) : 0
      return `${i+1},"${c.nome}",${c.total_leads},${c.leads_hot},${c.leads_warm},${c.leads_cold},${c.convertidos},${txConv}%,"${fmtMoeda(c.capital_total)}"`
    }).join('\n')
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {Array.from({ length: 12 }, (_, i) => {
                const m = 12 - i; const a = m > now.getMonth()+1 ? ANO-1 : ANO
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-[#10B981]">{consultoresFiltrados.length} consultores</span>
              </div>
              {consultoresFiltrados.length === 0 ? (
                <div className="py-16 text-center">
                  <p className="text-gray-600 text-sm">Nenhum dado para o período selecionado</p>
                </div>
              ) : (
              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {['#','Consultor','Leads','HOT','WARM','COLD','Conv','Taxa Conv','Capital'].map(h => (
                  ...
                  {consultoresFiltrados.map((c, i) => {
                    ...
                    return (
                      <tr key={c.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-3 font-bold text-yellow-400">{c.leads_warm}</td>
                        <td className="px-4 py-3 font-bold text-gray-400">{c.leads_cold}</td>
                        <td className="px-4 py-3 font-bold text-[#10B981]">{c.convertidos}</td>
                        <td className="px-4 py-3 font-bold text-[#10B981]">{txConv}%</td>
                        <td className="px-4 py-3 font-bold text-[#10B981]">{fmtMoeda(c.capital_total)}</td>
                      </tr>
                    )
                  })}
                  {!consultoresFiltrados.length && (
                    <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-600">Sem dados</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        {aba === 'filtros' && (
          <div className="max-w-lg space-y-5">
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-5">
              <p className="text-[9px] font-black uppercase tracking-wider text-gray-600 mb-4">Filtros Avançados</p>
              
              {/* Filtro por Categoria */}
              <div className="mb-4">
                <label className="block text-[10px] font-bold text-gray-400 mb-2">Categoria de Lead</label>
                <select
                  value={filtros.categoria}
                  onChange={e => setFiltros({ ...filtros, categoria: e.target.value })}
                  className="w-full bg-[#0F172A] border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#10B981]/50"
                >
                  <option value="todos">Todos</option>
                  <option value="hot">🔥 Hot (Score >= 80)</option>
                  <option value="warm">⚡ Warm (Score 60-79)</option>
                  <option value="cold">❄️ Cold (Score < 60)</option>
                </select>
              </div>

              {/* Filtro por Range de Leads */}
              <div className="mb-4">
                <label className="block text-[10px] font-bold text-gray-400 mb-2">Range de Leads</label>
                <div className="flex gap-2">
                  <input
                    type="number"
                    min={0}
                    value={filtros.minLeads}
                    onChange={e => setFiltros({ ...filtros, minLeads: parseInt(e.target.value) || 0 })}
                    placeholder="Mín"
                    className="flex-1 bg-[#0F172A] border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#10B981]/50"
                  />
                  <input
                    type="number"
                    min={0}
                    value={filtros.maxLeads}
                    onChange={e => setFiltros({ ...filtros, maxLeads: parseInt(e.target.value) || 999 })}
                    placeholder="Máx"
                    className="flex-1 bg-[#0F172A] border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#10B981]/50"
                  />
                </div>
              ...
              <button
                onClick={() => setFiltros({ categoria: 'todos', minLeads: 0, maxLeads: 999 })}
                className="w-full px-4 py-2 rounded-xl text-[11px] font-bold bg-white/5 text-white hover:bg-white/10 transition-all"
              >
                Limpar Filtros
              </button>
            </div>
