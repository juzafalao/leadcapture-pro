    ...
    <div className="fixed bottom-4 right-4 opacity-[0.08] pointer-events-none select-none z-0">
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          <div className="text-4xl font-black text-white leading-none mb-1">LeadCapture</div>
          <div className="text-2xl font-light text-white tracking-widest">PRO</div>
        </div>
      </div>
    </div>
  )
...
function TenantLogo({ tenant, selectedBrand, onBrandChange }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex items-center gap-3">
        {tenant.logo_url ? (
          <img src={tenant.logo_url} alt={tenant.name} className="h-12 w-auto max-w-[200px] object-contain rounded-lg bg-white/5 p-2" />
        ) : (
        ...
        <div>
          <h1 className="text-xl font-bold text-white">{tenant.name || 'LeadCapture Pro'}</h1>
      ...
      {hasBrands && brands.length > 1 && (
        <div className="flex items-center gap-2 ml-4">
          ...
          >
            ...
            {brands.map(brand => <option key={brand.id} value={brand.id}>{brand.icon} {brand.name}</option>)}
          </select>
        </div>
      )}
      ...
      {hasBrands && brands.length === 1 && (
        <div className="ml-4 px-3 py-1.5 bg-gray-800/50 border border-gray-700/50 rounded-lg">
          <span className="text-sm text-gray-400">{brands[0].icon} {brands[0].name}</span>
        </div>
      )}
    </div>
  )
...
function BrandBadge({ brand }) {
  ...
  return <span className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded text-xs font-medium">{brand.icon} {brand.name}</span>
}

function TenantSelector({ tenants, currentTenant, onSelect }) {
  if (tenants.length <= 1) return null
  return (
    <select value={currentTenant?.id || ''} onChange={(e) => onSelect(tenants.find(t => t.id === e.target.value))} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500">
...
function CRMIndicator({ tenant }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-800/50 px-3 py-1.5 rounded-lg border border-gray-700/50">
      ...
      <span className="text-xs font-medium text-gray-300">{crmConfig.icon} {crmConfig.name}</span>
    </div>
  )
}

// CRM Settings modal (keeps semi-transparent style)
function CRMSettingsModal({ tenant, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800/60 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700">
        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {Object.entries(CRM_INTEGRATIONS).map(([key, crm]) => (
                <button key={key} onClick={() => setSelectedCRM(key)} className={`p-4 rounded-xl border-2 transition ${selectedCRM === key ? 'border-orange-500 bg-orange-500/10' : 'border-gray-700 bg-gray-900/50 hover:border-gray-600'}`}>
          ...
          {selectedCRM && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">API Key / Token de Acesso</label>
              <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Cole sua API key aqui..." className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-orange-500" />
              <p className="text-xs text-gray-500 mt-2">💡 Esta chave será criptografada e armazenada com segurança</p>
            </div>
          )}
        ...
        <div className="p-6 border-t border-gray-700 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition">Cancelar</button>
          <button onClick={handleSave} disabled={!selectedCRM || !apiKey} className="px-6 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition disabled:opacity-50 disabled:cursor-not-allowed">💾 Salvar Configuração</button>
        </div>
      </div>
    </div>
  )
...
// MetricCard, StatusBadge, CategoryBadge, ScoreBadge, FonteBadge, Filters
function MetricCard({ title, value, icon, color, subtitle, onClick }) {
  ...
  return (
    <div className={className} onClick={onClick} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined} onKeyDown={handleKeyDown}>
...
function ScoreBadge({ score }) {
  ...
  return <span className={`px-3 py-1 rounded-full text-sm font-bold ${bgColor}`}>{numScore}</span>
}
    ...
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-gray-700/50 mb-6">
      <div className="flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Nome, email ou telefone..." value={filters.busca} onChange={(e) => setFilters({...filters, busca: e.target.value})} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500 placeholder-gray-500" />
        </div>

        {brands && brands.length > 1 && (
          <div className="w-36">
            <label className="block text-xs text-gray-400 mb-1">Marca</label>
            <select value={filters.brand_id} onChange={(e) => setFilters({...filters, brand_id: e.target.value})} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500">
        ...
        <div className="w-36">
          ...
          <select value={filters.status} onChange={(e) => setFilters({...filters, status: e.target.value})} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500">
        ...
        <div className="w-32">
          ...
          <select value={filters.categoria} onChange={(e) => setFilters({...filters, categoria: e.target.value})} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500">
            <option value="">Todos</option>
        ...
        <div className="w-36">
          ...
          <select value={filters.fonte} onChange={(e) => setFilters({...filters, fonte: e.target.value})} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500">
            <option value="">Todas</option>
            {fontes.map(f => <option key={f} value={f}>{FONTE_CONFIG[f]?.label || f}</option>)}
          </select>
        </div>

        <button onClick={() => setFilters({ busca: '', status: '', categoria: '', fonte: '', brand_id: '' })} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 transition text-sm">Limpar</button>
      </div>
    </div>
  )
}
...
function App() {
  ...
  const [filters, setFilters] = useState({ busca: '', status: '', categoria: '', fonte: '', brand_id: '' })

  // build statusOptions to pass to modal (value + label)
  ...
  useEffect(() => {
    async function loadTenants() {
      try {
        const { data, error } = await supabase.from('tenants').select('*').eq('active', true).order('name')
        if (error) throw error
        setTenants(data || [])
        if (data && data.length > 0) setCurrentTenant(data[0])
      } catch (err) {
  ...
  const refetchLeads = async () => {
    ...
    try {
      setLoading(true)
      const { data, error } = await supabase.from('leads').select('*').eq('tenant_id', currentTenant.id).order('created_at', { ascending: false })
      if (error) throw error
      setLeads(data || [])
    } catch (err) {
  ...
  // initial leads load (and on tenant change)
  useEffect(() => {
    if (!currentTenant) {
      setLoading(false)
      return
  ...
  const metrics = useMemo(() => {
    ...
    if (selectedBrand) filteredForMetrics = filteredForMetrics.filter(l => l.brand_id === selectedBrand.id)
  ...
  const filteredLeads = useMemo(() => {
    let result = leads
    if (selectedBrand) result = result.filter(l => l.brand_id === selectedBrand.id)
    if (activeTab === 'hot') result = result.filter(l => l.categoria?.toLowerCase() === 'hot')
    else if (activeTab === 'contatar') result = result.filter(l => ['novo', 'em_analise'].includes(l.status))
    else if (activeTab === 'convertidos') result = result.filter(l => l.status === 'convertido')

    if (filters.busca) {
      const busca = filters.busca.toLowerCase()
      result = result.filter(l => l.nome?.toLowerCase().includes(busca) || l.email?.toLowerCase().includes(busca) || l.telefone?.toLowerCase().includes(busca))
    }
    if (filters.brand_id) result = result.filter(l => l.brand_id === filters.brand_id)
    if (filters.status) result = result.filter(l => l.status === filters.status)
    if (filters.categoria) result = result.filter(l => l.categoria?.toLowerCase() === filters.categoria.toLowerCase())
    if (filters.fonte) result = result.filter(l => l.fonte === filters.fonte)
    return result
  ...
  const saveCRMConfig = async (crmData) => {
    try {
      const { error } = await supabase.from('tenants').update(crmData).eq('id', currentTenant.id)
      if (error) throw error
      setCurrentTenant({ ...currentTenant, ...crmData })
      const { data } = await supabase.from('tenants').select('*').eq('id', currentTenant.id).single()
  ...
  // Handle saving a lead (called by modal). Persist to Supabase and update local state with optimistic update + rollback.
  const handleLeadSave = async (updatedLead) => {
    ...
    try {
      const { data, error } = await supabase.from('leads').update({ status: updatedLead.status }).eq('id', updatedLead.id).select().single()
      if (error) throw error
      // update with DB canonical row
      setLeads(prev => prev.map(l => (String(l.id) === String(data.id) ? data : l)))
      setSelectedLead(null)
    } catch (err) {
      console.error('Erro ao atualizar lead no Supabase:', err)
      alert('Erro ao salvar no servidor: ' + (err.message || err))
      // rollback
      setLeads(prevLeads)
      await refetchLeads()
    }
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-orange-500 text-white rounded-lg">Tentar novamente</button>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-900 relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-6">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <TenantLogo tenant={currentTenant} selectedBrand={selectedBrand} onBrandChange={setSelectedBrand} />

          <div className="flex flex-wrap items-center gap-3">
            <CRMIndicator tenant={currentTenant} />

            <button onClick={() => setShowCRMSettings(true)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-300 hover:border-orange-500 hover:text-white transition text-sm" title="Configurar CRM">⚙️ CRM</button>

            <TenantSelector tenants={tenants} currentTenant={currentTenant} onSelect={setCurrentTenant} />

            <button onClick={() => setShowCharts(!showCharts)} className={`px-3 py-2 rounded-lg text-sm transition ${showCharts ? 'bg-orange-500 text-white' : 'bg-gray-700 text-gray-300'}`}>📊</button>

            <AppLogo />

            <div className="text-right">
              <p className="text-green-400 text-xs flex items-center gap-1"><span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> Online</p>
            </div>
          </div>
        </header>

        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
          <MetricCard title="Total de Leads" value={metrics.total} icon="📊" color="orange" subtitle={selectedBrand ? selectedBrand.name : 'Todas as marcas'} onClick={() => setActiveTab('todos')} />
          <MetricCard title="Leads HOT" value={metrics.hot} icon="🔥" color="red" onClick={() => setActiveTab('hot')} />
          <MetricCard title="A Contatar" value={metrics.aContatar} icon="📞" color="amber" onClick={() => setActiveTab('contatar')} />
          <MetricCard title="Convertidos" value={metrics.convertidos} icon="🎉" color="green" onClick={() => setActiveTab('convertidos')} />
          <MetricCard title="Taxa Conversão" value={`${metrics.taxaConversao}%`} icon="📈" color="purple" />
        </div>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${activeTab === tab.key ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{tab.label} ({tab.count})</button>
          ))}
        </div>

        <Filters filters={filters} setFilters={setFilters} leads={leads} brands={currentTenant?.brands} />

        <div className="flex justify-between items-center mb-4">
          <p className="text-gray-400 text-sm">Mostrando <span className="text-white font-bold">{filteredLeads.length}</span> leads {selectedBrand && <span className="text-orange-400"> • {selectedBrand.icon} {selectedBrand.name}</span>}</p>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-900/50 border-b border-gray-700/50">
                    ...
                    {currentTenant?.brands && currentTenant.brands.length > 1 && <th className="px-4 py-3 text-center text-xs font-semibold text-gray-400 uppercase">Marca</th>}
                ...
                <tbody className="divide-y divide-gray-700/30">
                  {filteredLeads.map((lead) => {
                    ...
                    return (
                      <tr key={lead.id} className="hover:bg-gray-700/30 transition-colors cursor-pointer" onClick={() => setSelectedLead(lead)}>
                        <td className="px-4 py-3"><p className="text-white font-medium">{lead.nome || '-'}</p></td>
                        {currentTenant?.brands && currentTenant.brands.length > 1 && <td className="px-4 py-3 text-center">{brand && <BrandBadge brand={brand} />}</td>}
                        <td className="px-4 py-3"><p className="text-gray-300 text-sm">{lead.email || '-'}</p><p className="text-gray-500 text-xs">{lead.telefone || '-'}</p></td>
                        <td className="px-4 py-3 text-center"><FonteBadge fonte={lead.fonte} /></td>
                        <td className="px-4 py-3 text-center"><ScoreBadge score={lead.score} /></td>
                        <td className="px-4 py-3 text-center"><CategoryBadge categoria={lead.categoria} /></td>
                        <td className="px-4 py-3 text-center"><StatusBadge status={lead.status} /></td>
                        <td className="px-4 py-3 text-right text-gray-400 text-sm">{formatDate(lead.created_at)}</td>
                      </tr>
                    )
                  })}
                </tbody>
      ...
      {showCRMSettings && <CRMSettingsModal tenant={currentTenant} onClose={() => setShowCRMSettings(false)} onSave={saveCRMConfig} />}
