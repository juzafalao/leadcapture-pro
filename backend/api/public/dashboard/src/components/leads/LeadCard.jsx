...
export function LeadCard({ lead, onClick, onWhatsApp }) {
  const marcaNome = lead.marca?.nome || 'Multimarcas';
  const marcaEmoji = lead.marca?.emoji || '🏢';
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1 truncate pr-4">
          <h3 className="font-bold text-[#f5f5f4] truncate text-sm">{lead.nome || 'Sem nome'}</h3>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-gray-500 font-black uppercase tracking-widest">
            {marcaEmoji} {marcaNome}
          </span>
          <button 
            onClick={(e) => { e.stopPropagation(); onWhatsApp(lead); }}
            className="p-1.5 bg-green-500/10 text-green-500 rounded-lg border border-green-500/20"
          >
